import {
  require_cjs
} from "./chunk-3FKAJUFY.js";
import {
  toJSONSchema
} from "./chunk-VVPRLPET.js";
import {
  __export,
  __toESM
} from "./chunk-OL46QLBJ.js";

// node_modules/nestjs-zod/dist/dto.mjs
...
var PREFIX = "x-nestjs_zod";
var EMPTY_TYPE_KEY = `${PREFIX}-empty-type`;
var DEFS_KEY = `${PREFIX}-$defs`;
var PARENT_ID_KEY = `${PREFIX}-parent-id`;
var PARENT_ADDITIONAL_PROPERTIES_KEY = `${PREFIX}-parent-additional-properties`;
var PARENT_HAS_REFS_KEY = `${PREFIX}-parent-has-refs`;
var UNWRAP_ROOT_KEY = `${PREFIX}-unwrap-root`;
var HAS_NULL_KEY = `${PREFIX}-has-null`;
function walkJsonSchema(schema, callback, options) {
  schema = callback((options == null ? void 0 : options.clone) ? (0, import_deepmerge.default)(schema, {}) : schema);
...
function is(input, factory) {
  const factories = external_exports;
...
function zodV3ToOpenAPI(zodType, visited = /* @__PURE__ */ new Set()) {
  ...
  if (is(zodType, external_exports.ZodString)) {
  ...
  if (is(zodType, external_exports.ZodBoolean)) {
    object.type = "boolean";
  }
  if (is(zodType, external_exports.ZodNumber)) {
    const { checks } = zodType._def;
    object.type = "number";
    for (const check of checks) {
      if (check.kind === "int") {
  ...
  if (is(zodType, external_exports.ZodBigInt)) {
    object.type = "integer";
    object.format = "int64";
  }
  if (is(zodType, external_exports.ZodArray)) {
  ...
  if (is(zodType, external_exports.ZodUnion)) {
    const { options } = zodType._def;
    object.oneOf = options.map((option) => zodV3ToOpenAPI(option, visited));
  }
  if (is(zodType, external_exports.ZodDiscriminatedUnion)) {
    const { options } = zodType._def;
    object.oneOf = [];
  ...
  if (is(zodType, external_exports.ZodLiteral)) {
  ...
  if (is(zodType, external_exports.ZodObject)) {
    ...
    for (const [key, schema] of Object.entries(shape())) {
      ...
      const optionalTypes = [external_exports.ZodOptional.name, external_exports.ZodDefault.name];
  ...
  if (is(zodType, external_exports.ZodRecord)) {
    const { valueType } = zodType._def;
    object.type = "object";
    object.additionalProperties = zodV3ToOpenAPI(valueType, visited);
  }
  if (is(zodType, external_exports.ZodIntersection)) {
    const { left, right } = zodType._def;
    const merged = (0, import_deepmerge.default)(
  ...
  if (is(zodType, external_exports.ZodEffects)) {
    const { schema } = zodType._def;
    Object.assign(object, zodV3ToOpenAPI(schema, visited));
  }
  if (is(zodType, external_exports.ZodLazy)) {
