...
const getTypicalCharWidth = (textItems: TextItems): number => {
  ...
  const commonTextItems = textItems.filter(
    (item) => item.height === commonHeight && item.fontName === commonFontName
  );
  const totalWidth = commonTextItems.reduce((acc, item) => acc + item.width, 0);
  const totalNumChars = commonTextItems.reduce(
    (acc, item) => acc + item.text.length,
    0
  );

  return totalWidth / totalNumChars;
};
